...
{
public:
  Global2Local(ros::NodeHandle &nh) : m_homeHandler(nh, "mavros/global_position/home", -1)
  {}
  ...
  Eigen::Vector3d toGlobal(const double t_enuX, const double t_enuY, const double t_enuZ)
  {
    ...
    Eigen::Vector3d localEcef =
      mavros::ftf::transform_frame_enu_ecef(localEnu, map_origin);
  ...
  Eigen::Vector3d toLocal(const double lat,
    const double lon,
    const double alt,
    bool altitudeRelative = false)
  {
